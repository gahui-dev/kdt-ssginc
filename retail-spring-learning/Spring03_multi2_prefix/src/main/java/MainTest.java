import org.springframework.context.ApplicationContext;
import org.springframework.context.support.GenericXmlApplicationContext;

import com.service.DeptServiceImpl;

public class MainTest {
	public static void main(String[] args) {
		// Spring 방식
		/*
		 * - 스프링에서는 생성된 클래스를 빈(bean) 이라고 부른다
		 * - 빈 생성 순서
		 * 	 1. 빈 작성(*.java)
		 * 
		 *   2. 빈을 xml에 등록 (src/main/resources, 패키지 사용 가능 )
		 *   	예) com.config / user.xml
		 *   
		 *   3. ApplicationContext 생성
		 *      - xml에 등록된 빈들의 lifecycle 관리(생성~소멸) 
		 *      - 빈 생성은 기본적으로 기본 생성자로 생
		 *    
		 *   4. 빈 얻기
		 *   	- 클래스 변수명 = ctx.getBean("id값", 클래스.class)
		 * */
		
		// 3. ApplicationContext 생성
		// window "file:\\Users\\leegahee\\user2.xml"
		ApplicationContext ctx = new GenericXmlApplicationContext("classpath:com/config/user.xml", "file:/Users/leegahee/user2.xml");
		
		
		/*
		 * user-config.xml
		 * user2-config.xml => *-config.xml
		 * webapp.xml
		 * */
		
		// 4. 빈 얻기
		DeptServiceImpl service = ctx.getBean("xxx", DeptServiceImpl.class);
		
		String mesg = service.getMesg();
		System.out.println("Spring 방식 >>" + mesg);
		
		// 출력 결과 : Spring 방식 : Hello World 홍길동 
		
	}
}
